#!/usr/bin/env python3
"""
ELITE UNIFIER NQ - Unifier spécialisé pour Chart 9 (NQ, RTY, CL)
===============================================================

Unifier robuste pour la famille NQ :
- NQ → Chart 9
- RTY → Chart 9  
- CL → Chart 9
...
class EliteUnifierNQ:
    """
    Unifier Elite spécialisé pour Chart 9 (NQ, RTY, CL)
    
    Fonctionnalités :
    - Même code Elite que l'unifier principal
    - QC robuste pour Chart 9
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialisation de l'Elite Unifier NQ"""
        if self._ready:
            return
            
        self.config = config or {}
        self.chart_number = 9
        self.supported_symbols = ["NQ", "RTY", "CL"]
        ...
        print(f"🚀 Elite Unifier NQ (Chart {self.chart_number}) initialisé")
    ...
    def unify_nq(self, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        """
        Unification robuste pour la famille NQ (Chart 9)
        ...
        try:
            # ✅ Vérifier que c'est bien un symbole NQ
            symbol = snapshot.get("sym", "NQ")
            if not self._is_nq_family(symbol):
                logger.warning(f"⚠️ Symbole {symbol} pas dans la famille NQ, redirection recommandée")
            
            # 1) Vérifier le QC journalier Chart 9
            date_str = datetime.now().strftime("%Y%m%d")
            day_ok, qc_reason = self._check_daily_qc_chart9(date_str)
            ...
            logger.info(f"🛡️ Unification NQ terminée: {robust_result['processing_time_ms']:.1f}ms")
            return robust_result
            
        except Exception as e:
            logger.error(f"❌ Erreur unification NQ: {e}")
            return {
                "timestamp": time.time(),
                "processing_time_ms": (time.perf_counter() - start_time) * 1000,
                "chart": self.chart_number,
                ...
                "message": f"❌ Erreur NQ: {e}"
            }
    
    def _is_nq_family(self, symbol: str) -> bool:
        """Vérifier si le symbole fait partie de la famille NQ"""
    ...
    def _check_daily_qc_chart9(self, date_str: str) -> Tuple[bool, str]:
        """Vérifier le QC journalier pour Chart 9"""
        try:
            ...
            if not qc_path.exists():
                # Throttle du warning (toutes les 5s)
                import time as _t
    ...
    def _compute_data_age_ms(self, snapshot: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if ts is None:
                return 0.0
            # ts peut être float (secondes) ou ISO
    ...
    def _unify_with_elite_methods(self, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Erreur traitement Elite NQ: {e}")
            base_ctx["elite_methods"] = {
                "available": False,
                "error": str(e),
                "chart": self.chart_number,
    ...
    def _process_menthorq_elite(self, ctx: Dict[str, Any], snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Erreur MenthorQ Elite NQ: {e}")
    ...
    def _process_battle_navale_elite(self, ctx: Dict[str, Any], snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Erreur Battle Navale Elite NQ: {e}")
            return {"score": 0.0, "error": str(e)}
    ...
    def _process_orderflow_advanced(self, ctx: Dict[str, Any], snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Erreur OrderFlow Advanced NQ: {e}")
            return {"score": 0.0, "error": str(e)}
    ...
    def _process_dom_health(self, ctx: Dict[str, Any], snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Erreur DOM Health NQ: {e}")
            return {"score": 0.0, "error": str(e)}
    ...
    def _synthesize_elite_results(self, menthorq_result: Dict, battle_navale_result: Dict, 
        ...
        try:
            ...
            logger.error(f"❌ Erreur synthèse Elite NQ: {e}")
            return {
                "composite_score": 0.0,
                "confidence": 0.0,
                "is_signal": False,
    ...
    def _build_clear_message(self, go_live: bool, elite_synthesis: Dict, day_ok: bool, qc_reason: str, symbol: str = "NQ") -> str:
        ...
        if go_live:
            ...
            return (f"✅ GO NQ | composite={composite:.3f} | "
    ...
    def _send_decision_message(self, robust_result: Dict[str, Any]):
        ...
        try:
            ...
            symbol = robust_result.get("snapshot", {}).get("sym", "NQ")
            ...
            logger.error(f"❌ Erreur envoi message NQ: {e}")
    
    def get_stats(self) -> Dict[str, Any]:
        """Obtenir les statistiques du unifier NQ"""
        return {
            "chart": self.chart_number,
            "symbol_family": "NQ",
...
# Fonction d'export
def unify_nq_elite(snapshot: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Fonction d'export pour unification NQ Elite
    ...
    Returns:
        Résultat de l'unification NQ Elite
    """
    unifier = EliteUnifierNQ(config)
    return unifier.unify_nq(snapshot)

# Test rapide
if __name__ == "__main__":
    print("🧪 Test Elite Unifier NQ...")
    ...
    test_snapshot = {
        ...
        "regime": "TREND",
        "vix": 18.5,
        "mentorq_gamma": {"levels": [], "call_wall": 18505.0, "put_wall": 18495.0},
        "mentorq_blind": {"spots": [], "spot_1": 18502.0, "spot_2": 18498.0},
        "vwap": {"value": 18500.0, "upper_band": 18505.0, "lower_band": 18495.0},
        "vp": {"vpoc": 18500.0, "val": 18495.0, "vah": 18505.0},
        "ofdom": {"best_bid": 18500.0, "best_ask": 18500.25, "spread": 0.25},
    ...
    # Test
    result = unify_nq_elite(test_snapshot)
    print(f"✅ Résultat NQ: {result['gating']['go_live']}")
