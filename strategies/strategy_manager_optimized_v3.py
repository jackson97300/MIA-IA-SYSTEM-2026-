...
try:
    from strategies.weekly_vwap_extreme_reversion import WeeklyVWAPExtremeReversion
    WEEKLY_VWAP_AVAILABLE = True
except ImportError:
    WEEKLY_VWAP_AVAILABLE = False

try:
    from strategies.initial_balance_breakout import InitialBalanceBreakout
    IB_BREAKOUT_AVAILABLE = True
except ImportError:
    IB_BREAKOUT_AVAILABLE = False

try:
    from strategies.pvwap_magnetic_bounce import PVWAPMagneticBounce
    PVWAP_MAGNETIC_AVAILABLE = True
except ImportError:
    PVWAP_MAGNETIC_AVAILABLE = False

try:
    from strategies.vpoc_extreme_reversion import VPOCExtremeReversion
    VPOC_EXTREME_AVAILABLE = True
except ImportError:
    VPOC_EXTREME_AVAILABLE = False

try:
    from strategies.next_wall_micro_zone_scalp import NextWallMicroZoneScalp
    NEXT_WALL_SCALP_AVAILABLE = True
except ImportError:
    NEXT_WALL_SCALP_AVAILABLE = False
...
class OptimizedStrategyManagerV3:
    ...
    def _load_strategies(self):
        ...
        # 🚫 DÉSACTIVÉ (25/11/2025) - On garde que menthorq_3layer avec TP/SL intelligents
        ...
        # 🚫 DÉSACTIVÉ (25/11/2025) - TP/SL fixes trop courts, on garde que menthorq_3layer
        ...
        # 🚫 DÉSACTIVÉ (25/11/2025)
        # if "weekly_vwap_extreme_reversion" in enabled and WEEKLY_VWAP_AVAILABLE:
        #     self.strategies['weekly_vwap_extreme_reversion'] = WeeklyVWAPExtremeReversion()
        #     logger.info("✅ Stratégie: weekly_vwap_extreme_reversion (Priority: 9)")

        # 🚫 DÉSACTIVÉ (25/11/2025)
        # if "initial_balance_breakout" in enabled and IB_BREAKOUT_AVAILABLE:
        #     self.strategies['initial_balance_breakout'] = InitialBalanceBreakout()
        #     logger.info("✅ Stratégie: initial_balance_breakout (Priority: 8)")

        # 🚫 DÉSACTIVÉ (25/11/2025)
        # if "pvwap_magnetic_bounce" in enabled and PVWAP_MAGNETIC_AVAILABLE:
        #     self.strategies['pvwap_magnetic_bounce'] = PVWAPMagneticBounce()
        #     logger.info("✅ Stratégie: pvwap_magnetic_bounce (Priority: 7)")

        # 🚫 DÉSACTIVÉ (25/11/2025)
        # if "vpoc_extreme_reversion" in enabled and VPOC_EXTREME_AVAILABLE:
        #     self.strategies['vpoc_extreme_reversion'] = VPOCExtremeReversion()
        #     logger.info("✅ Stratégie: vpoc_extreme_reversion (Priority: 8)")

        # 🚫 DÉSACTIVÉ (25/11/2025)
        # if "next_wall_micro_zone_scalp" in enabled and NEXT_WALL_SCALP_AVAILABLE:
        #     self.strategies['next_wall_micro_zone_scalp'] = NextWallMicroZoneScalp()
        #     logger.info("✅ Stratégie: next_wall_micro_zone_scalp (Priority: 9) ⚡ GAME CHANGER")

        logger.info(f"\n📊 Total stratégies chargées: {len(self.strategies)}")
        ...
        try:
            ...
            if self.rules_engine:
                ...
                if not passed:
                    ...
                    if self.rules_shadow_mode:
                        ...
                        logger.error(f"🔴 [AUDIT] [{symbol}] BLOQUÉ NIVEAU 1: RulesEngine REJET - {rejection_summary}")
                        logger.error(f"🔴 [AUDIT] Rule results: {rule_results}")
                        self.global_stats['total_rules_rejections'] += 1
                        return None
            ...
            if self.market_context_analyzer:
                try:
                    ...
                    if market_context and hasattr(market_context, 'quality_score'):
                        logger.info(f"📊 [AUDIT] [{symbol}] Market Context Quality: {market_context.quality_score:.2f}")
                        if market_context.quality_score < 0.3:
                            logger.error(f"🔴 [AUDIT] [{symbol}] BLOQUÉ NIVEAU 2: Market Context Quality trop faible ({market_context.quality_score:.2f} < 0.3)")
            ...
            for strategy_name in priority_order:
                ...
                if self._is_in_cooldown(strategy_name, current_time, symbol, current_context_str):
                    ...
                    logger.warning(f"⏰ [AUDIT] [{symbol}] {strategy_name}: BLOQUÉ NIVEAU 3 - Cooldown actif")
                    continue

                # Check session
                if not is_session_allowed(strategy_name, current_session):
                    self.stats[strategy_name]['session_blocks'] += 1
                    logger.warning(f"📅 [AUDIT] [{symbol}] {strategy_name}: BLOQUÉ NIVEAU 4 - Session non autorisée (session={current_session})")
                    continue
                else:
                    logger.debug(f"✅ [AUDIT] [{symbol}] {strategy_name}: Session OK ({current_session})")
                ...
                if signal and self.ml_3layer_system_to_inject:
                    logger.info(f"🤖 [ML DEBUG] [{symbol}] Avant _apply_ml_filters_global pour {strategy_name}")
                    signal = self._apply_ml_filters_global(signal, ml_data, symbol, strategy_name)
                    logger.info(f"🤖 [ML DEBUG] [{symbol}] Après _apply_ml_filters_global: signal={'OK' if signal else 'REJETÉ'}")

                    if not signal:
                        logger.error(f"🔴 [AUDIT] [{symbol}] {strategy_name}: BLOQUÉ NIVEAU 9 - Signal REJETÉ par filtres ML globaux")
                ...
                if signal:
                    ...
                    if signal_direction:
                        ...
                        if ml_confidence < ml_threshold:
                            logger.error(f"🔴 [AUDIT] [{symbol}] {strategy_name}: BLOQUÉ NIVEAU 11 - ML confidence trop faible ({ml_confidence:.4f} < {ml_threshold:.2f})")
                        ...
                        if symbol == 'ES' or symbol.startswith('ES'):
                            ...
                            # 🔧 TEST 20/11: Seuil réduit de 0.55 → 0.30 pour permettre plus de trades
                            # Le seuil de 0.55 était trop restrictif (0% de trades ES)
                            # 0.30 permet de tester si le problème vient de ce seuil
                            min_confluence_es = 0.30  # 🔧 TEST: Réduit de 0.55 → 0.30
                            if confluence < min_confluence_es:
                                logger.error(f"🔴 [AUDIT] [{symbol}] {strategy_name}: BLOQUÉ NIVEAU 12 - Confluence ES trop faible ({confluence:.4f} < {min_confluence_es:.2f})")
            ...
            if not all_signals:
                logger.error(f"🔴 [AUDIT] [{symbol}] BLOQUÉ NIVEAU 13: all_signals VIDE - Aucune stratégie n'a généré de signal")
                logger.error(f"🔴 [AUDIT] [{symbol}] Total évaluations: {self.global_stats['total_evaluations']}, Session: {current_session}")
                logger.error(f"🔴 [AUDIT] [{symbol}] Stratégies évaluées: {len(priority_order)}")
                # Logger stats de chaque stratégie
                for strategy_name in priority_order:
                    stats = self.stats[strategy_name]
                    logger.error(f"🔴 [AUDIT] [{symbol}] {strategy_name}: evaluations={stats['evaluations']}, cooldown_blocks={stats.get('cooldown_blocks', 0)}, session_blocks={stats.get('session_blocks', 0)}, ml_rejections={stats.get('ml_rejections', 0)}, confluence_rejections={stats.get('confluence_rejections', 0)}")
