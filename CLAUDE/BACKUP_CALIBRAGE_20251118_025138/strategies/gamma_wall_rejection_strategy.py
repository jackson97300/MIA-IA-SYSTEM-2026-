...
class GammaWallRejectionStrategy:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.max_distance_ticks = self.config.get('max_distance_ticks', 5)
        self.min_wall_strength = self.config.get('min_wall_strength', 0.25)
        self.min_rejection_range_ticks = self.config.get('min_rejection_range_ticks', 3)
        ...
        self.cooldown_seconds = self.config.get('cooldown_seconds', 60)  # 1 minute
        ...
        try:
            ...
            if call_resistance > 0:
                ...
                if dist_to_call <= self.max_distance_ticks:
                    ...
                    # 1. High proche call resistance (< 3 ticks)
                    # 2. Close loin du high (wick top >= 2 ticks)
                    # 3. Range suffisant (>= 3 ticks)
                    high_near_call = abs(high_price - call_resistance) / tick_size <= 3
                    wick_rejection = wick_top >= 2
            ...
            if put_support > 0:
                ...
                if dist_to_put <= self.max_distance_ticks:
                    ...
                    # 1. Low proche put support (< 3 ticks)
                    # 2. Close loin du low (wick bottom >= 2 ticks)
                    # 3. Range suffisant (>= 3 ticks)
                    low_near_put = abs(low_price - put_support) / tick_size <= 3
                    wick_rejection = wick_bottom >= 2
                    range_sufficient = candle_range >= self.min_rejection_range_ticks
