...
class MenthorQ3LayerStrategy:
    ...
    def __init__(self, ml_3layer_system=None):
        ...
        # ✅ CORRIGÉ 17/11: ES SL augmenté à 18 ticks et TP à 20 ticks (R:R 1.11:1)
        # ✅ CORRIGÉ 17/11: NQ SL augmenté à 18 ticks (au lieu de 12) pour réduire stop hunts
        self.sl_optimal_ticks = {
            'ES': 18,   # ✅ CORRIGÉ 17/11: 18 ticks = $225 (augmenté de 12 pour réduire hits prématurés)
            'NQ': 18,   # ✅ CORRIGÉ 17/11: 18 ticks = $90 (augmenté de 12 pour réduire stop hunts - MAE moyenne -17.6t)
            'RTY': 20   # $20 (inchangé, pas encore optimisé)
        }

        self.tp_optimal_ticks = {
            'ES': 20,   # ✅ CORRIGÉ 17/11: 20 ticks = $250 (R:R 1.11:1 - justifié par confluence >0.70)
            'NQ': 23,   # $115 (vs 50-60t avant) - Validé par optimisation
            'RTY': 25   # $25 (estimation)
        }
        ...
        # ✅ CORRECTION P0 #3: Seuil confluence augmenté (17/11/2025)
        # Revenir à 35% pour filtrer signaux de mauvaise qualité
        # Note: 28% permettait trop de trades avec Layer 1=0%, OrderFlow=1%
        self.min_total_confidence = 0.35   # 35% global (vs 28% avant) ✅
    ...
    def generate_signal(self, ml_data: Dict, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            # ═══════════════════════════════════════════════════════════════
            if symbol == 'ES':
                logger.info(f"🔍 [ES DEBUG] Appel ml_3layer_system.evaluate_signal()...")
            ...
            if isinstance(menthor_distances, dict):
                for key in ['near_gex_up', 'near_gex_dn', 'hvl0', 'near_blind']:
                    dist = menthor_distances.get(key, 999)
                    if isinstance(dist, (int, float)) and dist < closest_level_distance:
                        closest_level_distance = dist

            # Rejeter si aucun niveau proche
            if closest_level_distance > 10:
                logger.warning(
                    f"❌ [{symbol}] REJECT: Pas de niveau MenthorQ proche "
                    f"(closest={closest_level_distance:.1f}t > 10t)"
                )
            ...
            # VALIDATION #4: Layer 1 (MenthorQ) >= 0.20 (✅ CORRECTION P0 #1: 17/11/2025)
            # Layer 1 représente 50% du score total et doit être significatif
            layer1_confidence = result.get('layer1_confidence', 0.0)
            min_layer1_threshold = 0.20  # Minimum 20% pour Layer 1 (vs 0.15 avant)
