@echo off
:: MIA BOT - DEMARRAGE EN MODE TEST (Session Quality 24/7)
:: Active test_mode=True pour trader sans restriction horaire

title MIA - START BOT (MODE TEST)
chcp 65001 >nul

echo.
echo ================================================================
echo         DEMARRAGE DU BOT MIA - MODE TEST 24/7
echo ================================================================
echo.
echo   Ce mode permet de trader SANS restriction horaire
echo   (Session Quality desactive)
echo.
...
:: ETAPE 2: Activer MODE TEST dans le fichier
echo [2/5] Activation du MODE TEST dans le code...
powershell -Command "(Get-Content 'LAUNCH\launch_production_CLEAN_v2.py') -replace 'test_mode=False.*MODE PRODUCTION', 'test_mode=True  # MODE TEST: Trading 24/7 pour tests' | Set-Content 'LAUNCH\launch_production_CLEAN_v2.py'"
echo       test_mode = True (MODE TEST active)
echo.
...
echo [4/5] Demarrage du bot MIA en MODE TEST...
if not exist "logs" mkdir logs

:: Lancer avec logs visibles dans un fichier
...
echo       Bot lance en arriere-plan (MODE TEST)
...
echo ================================================================
if not "%RUNNING%"=="0" (
    echo   [OK] BOT DEMARRE EN MODE TEST !
    echo ----------------------------------------------------------------
    echo   Mode: TEST 24/7 (pas de restriction horaire)
    echo   Processus Python: %RUNNING%
    echo.
    echo   ATTENTION: Le bot trade a TOUTE heure!
    echo   Pour repasser en PRODUCTION: START_BOT_PRODUCTION.bat
    echo.
    echo   Commandes:
