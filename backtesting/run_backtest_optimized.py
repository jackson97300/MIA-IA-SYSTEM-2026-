#!/usr/bin/env python3
"""
Script de lancement du backtest MenthorQ OPTIMISE
Utilise la configuration optimisée basée sur les résultats du premier backtest
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'backtesting/results/backtest_optimized_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def main():
    """Execute backtest optimise"""

    logger.info("="*80)
    logger.info("BACKTEST MENTHORQ - VERSION OPTIMISEE")
    logger.info("="*80)

    # 1. Charger config optimisée
    config_path = Path(__file__).parent / 'config' / 'backtest_config_optimized.yaml'

    if not config_path.exists():
        logger.error(f"ERREUR: Fichier config optimise non trouve: {config_path}")
        logger.info("Utilisation de la config standard...")
    ...
    logger.info(f"  - Config SL/TP unique: SL={config.get('sl_configs', [{}])[0].get('ticks', 'N/A')} ticks, TP=RR {config.get('tp_configs', [{}])[0].get('ratio', 'N/A')}")
    logger.info(f"  - Confluence minimale: {config.get('filters', {}).get('min_confluence', 2)} niveaux")
    logger.info(f"  - Heures evitees: {config.get('trading_hours', {}).get('avoid', [])}")

    # 2. Run backtest
    logger.info("\nLancement du backtest optimise...")
    try:
    ...
    try:
        excel_path = Path('backtesting/results/backtest_results_optimized.xlsx')
    ...
    logger.info("\n" + "="*80)
    logger.info("OK: BACKTEST OPTIMISE TERMINE !")
