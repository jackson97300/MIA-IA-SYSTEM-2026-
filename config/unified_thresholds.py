...
🎯 SEUILS SYSTÈME MENTHORQ

⚠️ ATTENTION: Les valeurs de base sont maintenant importées depuis:
   config/trading_params.py (SOURCE UNIQUE DE VÉRITÉ)

   Ce fichier contient les OVERRIDES pour modes spéciaux (calibration, etc.)

Version: 2.1 - Centralisation dans trading_params.py
Date: 10 Décembre 2025
"""

# ═══════════════════════════════════════════════════════════════════════
# 🎯 IMPORT DEPUIS SOURCE UNIQUE - config/trading_params.py
# ═══════════════════════════════════════════════════════════════════════
from config.trading_params import (
    MIN_TOTAL_CONFIDENCE as BASE_MIN_TOTAL_CONFIDENCE,
    MIN_LAYER_CONFIDENCE as BASE_MIN_LAYER_CONFIDENCE,
    MAX_DISTANCE_TO_LEVEL,
    get_min_confidence as base_get_min_confidence,
    get_layer_min_confidence as base_get_layer_min_confidence,
)
...
CALIBRATION_MODE = False  # ✅ PRODUCTION MODE (02/12/2025)
...
if CALIBRATION_MODE:
    # ═══════════════════════════════════════════════════════════════════════
    # 🔥 OPTIMISÉ 02/12/2025 23:50 - REVUE SESSION
    # ═══════════════════════════════════════════════════════════════════════
    #
    # 📊 SESSION 02/12: 101 trades
    #    ES: 44 trades | WR 34.1% | P&L +$330
    #    NQ: 57 trades | WR 47.4% | P&L +$2,725
    #
    # 🚨 PROBLÈME CRITIQUE: Confluence 0.35 TROP BAS!
    #    4 trades confluence < 0.8 → Perte -$262.50
    #
    # 🎯 OBJECTIF: WR 50%+ | P&L +$5,000/jour | Réduire overtrading
    # Impact estimé: +$260/jour + réduction 101 → 60-70 trades/jour
    # ═══════════════════════════════════════════════════════════════════════

    # ═══════════════════════════════════════════════════════════════════════
    # 📊 SEUILS ÉQUILIBRÉS
    # 🔧 13/12: 1.00 était trop strict (bloquait 171 trades!)
    # ═══════════════════════════════════════════════════════════════════════
    MIN_TOTAL_CONFIDENCE = {
        'ES': 0.35,    # 🔧 13/12: RESTAURÉ valeur qui marchait
        'NQ': 0.35,    # 🔧 13/12: RESTAURÉ
        'RTY': 0.35    # 🔧 13/12: RESTAURÉ
    }

    # ═══════════════════════════════════════════════════════════════════════
    # 🎯 SEUILS PAR LAYER - OPTIMISÉS PAR INSTRUMENT
    # ═══════════════════════════════════════════════════════════════════════
    #
    # ES (E-mini S&P 500) - Liquide, réactif aux niveaux techniques:
    #   - MenthorQ CRITIQUE: ES suit parfaitement GEX/Gamma (0.70)
    #   - OrderFlow PERMISSIF: Trades gagnants même avec OF faible (0.08)
    #   - Context AJUSTÉ: VWAP moins discriminant sur ES (0.14)
    #
    # NQ (E-mini Nasdaq 100) - Volatil, sensible à l'orderflow:
    #   - MenthorQ IMPORTANT: Niveaux GEX importants sur tech (0.40)
    #   - OrderFlow CRITIQUE: NQ nécessite momentum directionnel (0.22)
    #   - Context AJUSTÉ: Légèrement moins strict (0.16)
    # ═══════════════════════════════════════════════════════════════════════

    # ═══════════════════════════════════════════════════════════════════════
    # 🎯 SEUILS CALIBRÉS SUR DONNÉES RÉELLES (71 trades analysés - 12/12/2025)
    # ═══════════════════════════════════════════════════════════════════════
    # DÉCOUVERTE: Haute confluence (>=1.1) = 31.6% WR (MAUVAIS!)
    #             Moyenne confluence (0.9-1.1) = 61.9% WR (BON!)
    #             Basse confluence (<0.9) = 70% WR (TRÈS BON!)
    # → Les seuils bas permettent de capturer les meilleurs trades!
    # ═══════════════════════════════════════════════════════════════════════
    # ⚠️ V10.2 16/12/2025: L2 = 0.15 PARTOUT (OrderFlow plus impactant)
    MIN_LAYER_CONFIDENCE = {
        'ES': {
            'layer1': 0.20,  # 🔧 V10.3: MenthorQ permissif
            'layer2': 0.17,  # 🔥 V10.3: OrderFlow = 0.17 (augmenté)
            'layer3': 0.12   # 🔧 V10.3: Context permissif
        },
        'NQ': {
            'layer1': 0.20,  # 🔧 V10.3: MenthorQ permissif
            'layer2': 0.17,  # 🔥 V10.3: OrderFlow = 0.17 (augmenté)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # 🔒 MODE PRODUCTION - IMPORTÉ DEPUIS config/trading_params.py
    # ⚠️ MODIFIER trading_params.py POUR CHANGER CES VALEURS!
    # ═══════════════════════════════════════════════════════════════════════
    MIN_TOTAL_CONFIDENCE = BASE_MIN_TOTAL_CONFIDENCE.copy()
    MIN_LAYER_CONFIDENCE = {
        sym: layers.copy() for sym, layers in BASE_MIN_LAYER_CONFIDENCE.items()
    }

# ═══════════════════════════════════════════════════════════════════════
# 🔥 NOUVEAUX FILTRES COMPLÉMENTAIRES (02/12/2025)
# ═══════════════════════════════════════════════════════════════════════

# Filtre Risk:Reward minimum par instrument
# 📊 ES R:R < 1.00 = 8 trades, WR 62.5%, P&L -$146 → BLOQUER
...
# 📊 ES distance > 50t = 0% WR observé
# ⚠️  S'applique à TOUS les niveaux: next_wall, GEX, gamma_walls, blind_spots, HVL, etc.
# Le bot vérifie la distance au niveau MenthorQ le PLUS PROCHE, peu importe son type
MAX_DISTANCE_TO_LEVEL = {
    'ES': 8,       # 🎯 10/12/2025: 2 pts - OPTIMAL (WR 64.7% avec TP15/SL15)
    'NQ': 10,      # 🎯 10/12/2025: Réduit de 15 à 10
    'RTY': 12      # RTY: Valeur originale
}

# Zone morte OrderFlow (rejeter si dans cette zone)
# 📊 NQ OrderFlow 0.10-0.15 = 30% WR seulement
ORDERFLOW_DEAD_ZONE = {
...
# ═══════════════════════════════════════════════════════════════════════
# 🆕 FILTRE PRESSURE_STRENGTH PAR SESSION (06/12/2025)
# ═══════════════════════════════════════════════════════════════════════
# Backtest sur 24 jours (Nov-Dec 2025): P&L +$3,125 → +$141,375 (+4,424%)
#
# Impact par session:
#   - LONDON: -$94,375 → +$6,000 (seuil 0.10 CRITIQUE)
#   - US_MORNING: +$86,375 → +$90,125 (seuil 0.03 optimal)
#   - POWER_HOUR: +$11,125 → +$52,375 (seuil 0.10 optimal)
#
# Le pressure_strength mesure la force de la pression directionnelle
# Un score faible = signal faible = trade à éviter
# ═══════════════════════════════════════════════════════════════════════

MIN_PRESSURE_STRENGTH_BY_SESSION = {
    'London': 0.10,        # 08:00-11:00 - Strict (session difficile, WR 24%)
    'US Morning': 0.03,    # 15:50-17:00 - Souple (session très rentable, WR 38%)
    'US Power Hour': 0.10, # 20:00-21:30 - Intermédiaire
    # Sessions non tradées (seuil haut pour bloquer)
    'ASIA': 0.50,          # 00:00-08:00 - Pas de trading
    'Pre-US': 0.50,        # 11:00-15:25 - Pas de trading
    'Lunch': 0.50,         # 17:00-19:30 - Pas de trading
    'Closed': 0.50,        # 21:30-00:00 - Pas de trading
}

# 🔥 OPTIMISÉ 07/12/2025: Seuil pressure SPÉCIFIQUE par SYMBOLE
# Backtest: ES avec pressure >= 0.20 améliore WR de +5% et P&L de +$1,700
MIN_PRESSURE_STRENGTH_BY_SYMBOL = {
    'ES': 0.20,    # 🔥 OPTIMISÉ: ES nécessite un pressure plus fort (backtest 07/12)
    'NQ': 0.03,    # ✅ NQ reste souple (très rentable avec seuil bas)
    'RTY': 0.10,   # ✅ Standard
}

# Seuil par défaut si session non reconnue
MIN_PRESSURE_STRENGTH_DEFAULT = 0.10


def get_min_pressure_strength(session_name: str, symbol: str = None) -> float:
    """
    Retourne le seuil minimum de pressure_strength.

    🔥 OPTIMISÉ 07/12/2025: Prend en compte le symbole
    - ES utilise un seuil plus strict (0.20) peu importe la session
    - NQ/RTY utilisent les seuils par session

    Args:
        session_name: Nom de la session (London, US Morning, US Power Hour, etc.)
        symbol: Symbole (ES, NQ, RTY) - optionnel

    Returns:
        float: Seuil minimum de pressure_strength
    """
    # Si symbole spécifié, utiliser le seuil du symbole s'il est plus strict
    if symbol and symbol in MIN_PRESSURE_STRENGTH_BY_SYMBOL:
        symbol_threshold = MIN_PRESSURE_STRENGTH_BY_SYMBOL[symbol]
        session_threshold = MIN_PRESSURE_STRENGTH_BY_SESSION.get(session_name, MIN_PRESSURE_STRENGTH_DEFAULT)
        # Prendre le MAX (le plus strict)
        return max(symbol_threshold, session_threshold)

    return MIN_PRESSURE_STRENGTH_BY_SESSION.get(session_name, MIN_PRESSURE_STRENGTH_DEFAULT)
...
__all__ = [
    ...
    'CONFLUENCE_CONFIG',
    'ML_THRESHOLD_BY_TIER',
    # 🔥 NOUVEAUX EXPORTS 02/12/2025
    'MIN_RISK_REWARD_RATIO',
    'MAX_DISTANCE_TO_LEVEL',
    'ORDERFLOW_DEAD_ZONE',
    # 🆕 EXPORTS 06/12/2025 - Pressure Strength
    'MIN_PRESSURE_STRENGTH_BY_SESSION',
    'MIN_PRESSURE_STRENGTH_BY_SYMBOL',  # 🔥 AJOUTÉ 07/12/2025
    'MIN_PRESSURE_STRENGTH_DEFAULT',
    'get_min_pressure_strength',
    # Helpers
    'get_min_confidence',
