...
class SierraDTCConnector:
    ...
    async def _cancel_order_by_client_id(self, symbol: str, client_order_id: str, reason: str = ""):
        ...
        try:
            ...
            # 🔥 STRATÉGIE : CANCEL + REPLACE MULTIPLE
            ...
            await asyncio.sleep(1.5)  # 🔥 DÉLAI ENCORE PLUS LONG (1.5s au lieu de 1.0s)
            
            # Étape 2: REPLACE x5 (au lieu de x3) avec délais plus longs
            for attempt in range(5):  # 🔥 5 tentatives au lieu de 3
                replace_msg = {
                    "Type": CANCEL_REPLACE_ORDER,  # Type 204
                    "RequestID": self.request_id_counter,
                    "ServerOrderID": server_order_id,
                    "ClientOrderID": client_order_id,
                    "TradeAccount": trade_account,
                    "Price1": 0.01,  # Prix très bas
                    "Price2": 0.01,
                    "Quantity": 0.0,  # Quantité 0 = invisible
                    "Price1IsSet": 1,
                    "Price2IsSet": 1,
                    "TimeInForce": 3  # TIF_IOC pour expiration immédiate
                }
                self.request_id_counter += 1
                
                logger.warning(f"🔥 [DTC->] REPLACE #{attempt+1}/5 {client_order_id} qty=0 price=0.01 TIF=IOC")
                await self._send_dtc_message(sock, replace_msg)
                await asyncio.sleep(0.7)  # 🔥 Pause encore plus longue (0.7s)

            logger.info(f"✅ Annulation + REPLACE x5 terminée pour {client_order_id}")
