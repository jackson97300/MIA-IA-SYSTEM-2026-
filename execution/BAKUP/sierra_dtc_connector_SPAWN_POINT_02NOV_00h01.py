...
class SierraDTCConnector:
    ...
    async def _cancel_order_by_client_id(self, symbol: str, client_order_id: str, reason: str = ""):
        ...
        try:
            ...
            await asyncio.sleep(0.5)  # Attente plus longue pour le traitement

            # Étape 2: REPLACE x3 pour forcer mise à jour DOM
            for attempt in range(3):
                replace_msg = {
                    ...
                    "ClientOrderID": client_order_id,
                    "TradeAccount": trade_account,
                    "Price1": 99999.0,  # Prix impossible
                    "Price2": 99999.0,
                ...
                logger.warning(f"🔥 [DTC->] REPLACE #{attempt+1}/3 {client_order_id} qty=0 TIF=IOC")
                await self._send_dtc_message(sock, replace_msg)
                await asyncio.sleep(0.3)  # Pause entre chaque REPLACE

            logger.info(f"✅ Annulation + REPLACE x3 terminée pour {client_order_id}")
