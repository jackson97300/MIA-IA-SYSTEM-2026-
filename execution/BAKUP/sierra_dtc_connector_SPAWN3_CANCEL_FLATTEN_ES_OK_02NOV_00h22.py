...
class SierraDTCConnector:
    ...
    async def _cancel_order_by_client_id(self, symbol: str, client_order_id: str, reason: str = ""):
        ...
        try:
            ...
            # 🔥 Étape 2: FLATTEN_ALL pour nettoyer TOUT le compte
            flatten_msg = {
                "Type": FLATTEN_POSITIONS_FOR_TRADE_ACCOUNT,  # Type 210 - Nettoie TOUT
                "RequestID": self.request_id_counter,
                "TradeAccount": trade_account
            }
            self.request_id_counter += 1
            
            logger.warning(f"🔥 [DTC->] FLATTEN_ALL compte {trade_account} pour nettoyer DOM")
            await self._send_dtc_message(sock, flatten_msg)
            
            logger.info(f"✅ CANCEL + FLATTEN_ALL terminée pour {client_order_id}")
