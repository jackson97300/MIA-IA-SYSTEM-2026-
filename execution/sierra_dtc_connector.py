...
class SierraDTCConnector:
    ...
    def __init__(self, config: DTCConfig):
        ...
        # Mapping symbol → port
        # ✅ 16/12/2025: Ajout H26 (Mars 2026) après rollover Z25 → H26
        self.symbol_ports = {
            "ES": self.config.es_port,
            "NQ": self.config.nq_port,
            # H26 - Mars 2026 (ACTIF depuis 11/12/2025)
            "ESH26": self.config.es_port,
            "NQH26": self.config.nq_port,
            "ESH26_FUT_CME": self.config.es_port,
            "NQH26_FUT_CME": self.config.nq_port,
            "ESH26-CME": self.config.es_port,
            "NQH26-CME": self.config.nq_port,
            # Z25 - Décembre 2025 (EXPIRÉ - gardé pour compatibilité)
            "ESZ25": self.config.es_port,
            "NQZ25": self.config.nq_port,
            "ESZ25_FUT_CME": self.config.es_port,
            "NQZ25_FUT_CME": self.config.nq_port,
            # U25 - Septembre 2025 (EXPIRÉ)
        ...
        # 🆕 25/11/2025: Callback pour notifier fills
        self._fill_callback = None

    def set_fill_callback(self, callback):
        """
        Enregistre un callback appelé quand un ordre TP/SL est filled.

        Args:
            callback: Fonction async(symbol, client_order_id, fill_price, exit_type)
        """
        self._fill_callback = callback
        logger.info("✅ Fill callback enregistré")
    ...
    async def flatten_all(self, symbol: str) -> bool:
        """
        Ferme toutes les positions ET annule tous les ordres d'un symbole

        🔥 CRITIQUE: Utilisé au démarrage pour nettoyer les ordres orphelins!
        ...
        try:
            if self.paper_mode:
                ...
                # En paper mode, annuler tous les ordres ouverts
                for order in self.paper_orders:
                    if order.get("symbol") == symbol and order.get("status") == "open":
                        order["status"] = "cancelled"
                return True

            # Mode réel - Envoyer SUBMIT_FLATTEN_POSITION_ORDER via DTC
            ...
            if not sock:
                ...
                sock = self.connections.get(key)
                if not sock:
                    logger.warning(f"flatten_all {symbol}: Connexion non disponible")
                    return False

            # Obtenir le trade_account correct pour ce symbole
            trade_account = self._account_for_symbol(sc_symbol)

            # Envoyer FLATTEN_POSITION pour ce symbole
            flatten_msg = {
            ...
            if success:
                logger.info(f"✅ FLATTEN_ALL {symbol} envoyé (compte: {trade_account}) - Ordres annulés")
            else:
                logger.error(f"❌ FLATTEN_ALL {symbol} échec d'envoi")
    ...
    async def get_all_positions(self) -> List[Dict[str, Any]]:
        """
        Récupère toutes les positions ouvertes (tous symboles)
        ...
        try:
            if self.paper_mode:
                # En PAPER MODE, pas de positions réelles
                return []

            # Mode réel - Pour l'instant, retourner liste vide
            # TODO: Implémenter récupération positions via DTC si nécessaire
            # Le DTC peut envoyer des Position Update messages (Type 211)
            # mais nécessite un tracking des positions reçues
            logger.debug("get_all_positions: Non implémenté (retourne [])")
            return []
    ...
    async def close_position(self, symbol: str) -> bool:
        """
        Ferme une position (appelé après que TP/SL ait été touché via OCO)

        Note: En pratique, la position est déjà fermée via le mécanisme OCO (TP/SL).
        Cette méthode est un "safety net" pour nettoyer les ordres orphelins éventuels.

        Args:
            symbol: Symbole de la position à fermer

        Returns:
            True si succès (ou pas de position à fermer)
        """
        try:
            if self.paper_mode:
                logger.debug(f"[PAPER] close_position {symbol} - ignoré")
                return True

            # La position est normalement déjà fermée via OCO
            # On appelle flatten_all pour nettoyer tout ordre orphelin
            logger.debug(f"✅ [{symbol}] close_position appelé (position déjà fermée via OCO)")

            # NE PAS appeler flatten_all ici car:
            # 1. La position est déjà fermée par le TP/SL OCO
            # 2. flatten_all pourrait interférer avec d'autres ordres
            # Cette méthode est juste un "no-op" de sécurité

            return True

        except Exception as e:
            logger.error(f"❌ Erreur close_position {symbol}: {e}")
            return False

    # ═══════════════════════════════════════════════════════════════
    # 🔧 NOUVEAU 27/11/2025: MODIFICATION STOP LOSS EN COURS DE TRADE
    # ═══════════════════════════════════════════════════════════════

    async def modify_stop_loss(self, symbol: str, client_order_id: str,
                               new_sl_price: float, trade_account: str = None) -> bool:
        """
        Modifie un stop loss existant via CANCEL_REPLACE_ORDER (Type 204)

        Args:
            symbol: Symbole (ES, NQ, RTY)
            client_order_id: ClientOrderID du SL à modifier
            new_sl_price: Nouveau prix du SL
            trade_account: Compte de trading (optionnel, auto-détecté si None)

        Returns:
            True si succès, False sinon
        """
        try:
            # Déterminer le symbole Sierra Chart
            sc_symbol = self._to_sierra_trading_symbol(symbol)
            sock = self.connections.get(sc_symbol)

            if not sock:
                logger.error(f"❌ [{symbol}] Pas de connexion DTC pour modifier SL {client_order_id}")
                return False

            # Récupérer ServerOrderID si disponible
            server_order_id = self._server_order_ids.get(client_order_id, "")

            # Auto-détection du trade account
            if trade_account is None:
                trade_account = self._account_for_symbol(sc_symbol)

            # Construire message CANCEL_REPLACE_ORDER (Type 204)
            # Documentation DTC: https://dtcprotocol.org/index.php?page=doc/DTCMessageDocumentation.php
            replace_msg = {
                "Type": CANCEL_REPLACE_ORDER,  # 204
                "RequestID": self.request_id_counter,
                "ServerOrderID": server_order_id,
                "ClientOrderID": client_order_id,
                "Price1": new_sl_price,  # Nouveau prix du Stop Loss
                "Price2": 0.0,           # Non utilisé pour STOP
                "Quantity": 0,           # 0 = garder la quantité originale
                "OrderType": 3,          # OT_STOP = 3
                "TimeInForce": 1,        # TIF_GTC = 1
                "TradeAccount": trade_account
            }
            self.request_id_counter += 1

            logger.info(f"📈 [{symbol}] DTC CANCEL_REPLACE SL: {client_order_id} → {new_sl_price:.2f}")
            logger.debug(f"   ServerOrderID: {server_order_id}, TradeAccount: {trade_account}")

            # Envoyer le message
            success = await self._send_dtc_message(sock, replace_msg)

            if success:
                logger.info(f"✅ [{symbol}] Demande modification SL envoyée: {new_sl_price:.2f}")
            else:
                logger.warning(f"⚠️ [{symbol}] Échec envoi modification SL")

            return success

        except Exception as e:
            logger.error(f"❌ [{symbol}] Erreur modification SL {client_order_id}: {e}")
            import traceback
                ...
                if t == 301:  # ORDER_UPDATE
                    ...
                    if order_status == 7 and client_order_id in self._oco_pairs:
                        if client_order_id not in self._oco_processed:
                            ...
                            # ✅ CORRECTION CRITIQUE 27/11: skip_flatten=True car position déjà fermée par le FILL
                            # Quand un ordre OCO est FILLED, Sierra Chart ferme automatiquement la position
                            # Le FLATTEN est inutile et peut causer des problèmes
                            asyncio.create_task(self._cancel_order_by_client_id(
                                symbol=msg_symbol,
                                client_order_id=opposite_cid,
                                reason=f"OCO: {client_order_id} filled",
                                skip_flatten=True  # ✅ Position déjà fermée par Sierra Chart
                            ))

                            # 🆕 25/11/2025: NOTIFIER LE BOT DU FILL !
                            if hasattr(self, '_fill_callback') and self._fill_callback:
                                try:
                                    # Déterminer si TP ou SL
                                    exit_type = "TP" if "_TP_" in client_order_id else "SL"
                                    fill_price = msg.get("LastFillPrice", 0.0) or msg.get("Price1", 0.0)

                                    logger.info(f"📞 [CALLBACK] Appel fill_callback: {msg_symbol} {exit_type} @ {fill_price}")
                                    asyncio.create_task(self._fill_callback(
                                        symbol=msg_symbol,
                                    ...
                                    logger.error(f"❌ Erreur callback fill: {e}")
    ...
    async def _cancel_order_by_client_id(self, symbol: str, client_order_id: str, reason: str = "", skip_flatten: bool = False):
        """
        Annule un ordre par son ClientOrderID

        🔥 NOUVELLE APPROCHE:
        - Essaie CANCEL classique
        - Si skip_flatten=False, utilise FLATTEN pour tout retirer
        - Si skip_flatten=True, envoie seulement CANCEL (pour OCO quand position déjà fermée)

        Args:
            symbol: Symbole de l'ordre
            client_order_id: ClientOrderID de l'ordre à annuler
            reason: Raison de l'annulation (pour logs)
            skip_flatten: Si True, ne pas envoyer FLATTEN (pour OCO quand position déjà fermée)
        """
        try:
            sc_symbol = self._to_sierra_trading_symbol(symbol)
            key = sc_symbol
            ...
            await asyncio.sleep(0.5)  # 🔥 Attente après CANCEL

            # ✅ CORRECTION CRITIQUE 27/11: Ne pas envoyer FLATTEN si skip_flatten=True
            # Quand un ordre OCO est FILLED, la position est déjà fermée par Sierra Chart
            # Le FLATTEN est inutile et peut causer des problèmes
            if not skip_flatten:
                ...
                logger.info(f"✅ CANCEL terminée pour {client_order_id} (FLATTEN ignoré - position déjà fermée)")
    ...
    async def place_parent_then_children(
        ...
        *,
        ...
        🔥 25/11/2025: Support offsets relatifs
        - Si use_offsets=True: utilise tp_offset_ticks et sl_offset_ticks
        - Si use_offsets=False: utilise tp_price et sl_price (legacy)
        """
        await self.ensure_connected(symbol)
        ...
        if mode_str in ("oco206", "childrenmode.oco206"):
            ...
            # 🔥 25/11/2025: RÉCUPÉRER LE PRIX DE FILL RÉEL DU PARENT
            parent_ack = self._pending_acks.get(parent_cid)
            if parent_ack and parent_ack.get("OrderStatus") == 7:  # FILLED
                # Prix de fill réel (LastFillPrice ou LastPrice ou Price1)
                actual_fill_price = parent_ack.get("LastFillPrice") or parent_ack.get("LastPrice") or parent_ack.get("Price1") or 0.0
                logger.info(f"🔥 [{sc_symbol}] Prix de fill parent réel: {actual_fill_price:.2f}")
            else:
                logger.warning(f"⚠️ [{sc_symbol}] Prix de fill parent non disponible, utilisation entry_price théorique")
                actual_fill_price = entry_price if entry_price else 0.0

            # 🔥 25/11/2025: Support offsets OU prix absolus
            # EN MODE SEPARATE, ON DOIT CALCULER LES PRIX ABSOLUS DEPUIS LE FILL RÉEL !
            if use_offsets:
                if tp_offset_ticks is None or sl_offset_ticks is None:
                    raise ValueError("use_offsets=True requires tp_offset_ticks and sl_offset_ticks")

                # Tick size pour NQ/ES = 0.25
                tick_size = 0.25
                parent_side = (side or "").upper()

                # 🔥 FIX: Calcul selon la direction!
                # LONG (BUY): TP au-dessus (+), SL en-dessous (-)
                # SHORT (SELL): TP en-dessous (-), SL au-dessus (+)
                if parent_side == "BUY":  # LONG
                    tp_price_calculated = actual_fill_price + (tp_offset_ticks * tick_size)
                    sl_price_calculated = actual_fill_price - (sl_offset_ticks * tick_size)
                else:  # SHORT (SELL)
                    tp_price_calculated = actual_fill_price - (tp_offset_ticks * tick_size)
                    sl_price_calculated = actual_fill_price + (sl_offset_ticks * tick_size)

                logger.info(f"🔥 [{sc_symbol}] Mode OFFSETS → PRIX ABSOLUS ({parent_side}):")
                logger.info(f"   Fill réel: {actual_fill_price:.2f}")
                logger.info(f"   TP: {tp_price_calculated:.2f} ({tp_offset_ticks}t {'au-dessus' if parent_side == 'BUY' else 'en-dessous'})")
                logger.info(f"   SL: {sl_price_calculated:.2f} ({sl_offset_ticks}t {'en-dessous' if parent_side == 'BUY' else 'au-dessus'})")

                tp_msg_price = {"Price1": float(tp_price_calculated), "Price2": 0.0}
                sl_msg_price = {"Price1": float(sl_price_calculated), "Price2": 0.0, "StopPrice": float(sl_price_calculated)}
            else:
                if tp_price is None or sl_price is None:
                    raise ValueError("use_offsets=False requires tp_price and sl_price")
                logger.info(f"🔥 [{sc_symbol}] Mode PRIX ABSOLUS: TP={tp_price:.2f}, SL={sl_price:.2f}")
                tp_msg_price = {"Price1": float(tp_price), "Price2": 0.0}
                sl_msg_price = {"Price1": float(sl_price), "Price2": 0.0, "StopPrice": float(sl_price)}

            # ✅ BACKUP DOC: NE PAS utiliser ParentTriggerClientOrderID - ça casse tout!
            tp_msg = {
                "Type": SUBMIT_NEW_SINGLE_ORDER,
                "RequestID": self.request_id_counter,
                "Symbol": sc_symbol,
                ...
                "Quantity": float(qty),
                **tp_msg_price,  # ✅ Prix absolus calculés
                "TimeInForce": TIF_DAY,
                "ClientOrderID": tp_cid,
                "TradeAccount": trade_account,
                # ❌ PAS DE ParentTriggerClientOrderID - ne fonctionne pas!
            ...
            sl_msg = {
                ...
                "BuySell": child_side_code,
                "Quantity": float(qty),
                **sl_msg_price,  # ✅ Prix absolus calculés
                "TimeInForce": TIF_DAY,
