...
class PostMortemAnalyzer:
    ...
    def start_post_mortem_tracking(self,
                                  ...
                                  current_market_data):  # ✅ MODIFIÉ: Accept Dict OU MarketData
        """
        ...
        try:
            # ═══════════════════════════════════════════════════════════════
            # ✅ ADAPTATION ML_READY: Convertir Dict → MarketData si nécessaire
            # ═══════════════════════════════════════════════════════════════
            if isinstance(current_market_data, dict):
                # C'est un dict ML_READY, on crée un MarketData temporaire
                market_data = MarketData(
                    symbol=current_market_data.get('sym', 'UNKNOWN'),
                    timestamp=datetime.fromtimestamp(current_market_data.get('t_ms', 0) / 1000.0, tz=timezone.utc),
                    open=current_market_data.get('mid', 0),  # Fallback
                    high=current_market_data.get('mid', 0),  # Fallback
                    low=current_market_data.get('mid', 0),   # Fallback
                    close=current_market_data.get('mid', 0),  # Prix actuel
                    volume=0  # Non disponible dans ML_READY
                )
                logger.debug("✅ Dict ML_READY converti en MarketData pour PostMortem")
            else:
                # C'est déjà un MarketData, on l'utilise tel quel
