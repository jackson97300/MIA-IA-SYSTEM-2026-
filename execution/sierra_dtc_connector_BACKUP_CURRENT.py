...
class SierraDTCConnector:
                ...
                if t == 301:  # ORDER_UPDATE
                    ...
                    if order_status == 7 and client_order_id in self._oco_pairs:
                        if client_order_id not in self._oco_processed:
                            ...
                            logger.warning(f"🚨 {client_order_id} FILLED → Annulation automatique {opposite_cid}")
                            self._oco_processed.add(client_order_id)

                            # 🔥 Annuler l'ordre opposé avec le bon symbole (du message)
                            asyncio.create_task(self._cancel_order_by_client_id(
                                symbol=msg_symbol,  # 🔥 Symbole du MESSAGE, pas de la connexion !
    ...
    async def place_parent_then_children(
        ...
        *,
        ...
        try:
            # 🔥 STRATÉGIE "WAIT & FIRE" selon type d'ordre
            if entry_kind == "MKT":
                # MARKET : Attendre le fill (5s max - devrait être instantané)
                timeout_attempts = 50  # 5s
                for attempt in range(timeout_attempts):
                    ...
                    if parent_ack:
                        ...
                        if order_status == 7:  # FILLED
                            parent_filled = True
                            logger.info(f"✅ Parent MARKET FILLED - Position ouverte !")
                            break
                ...
                if not parent_filled:
                    logger.warning(f"⚠️ Parent MARKET non FILLED après 5s - Envoi TP/SL quand même")
            
            else:
                # 🔥 LIMIT/STOP : WAIT & FIRE (attendre le fill obligatoirement)
                # En simulation locale, ParentTriggerClientOrderID ne fonctionne pas
                # Les TP/SL s'exécutent immédiatement même si parent pas rempli
                # Donc on DOIT attendre le fill du parent !
                timeout_attempts = 600  # 60s max
                
                logger.warning(f"⏳ WAIT & FIRE: Attente FILL parent LIMIT (max 60s)...")
                logger.warning(f"⚠️  En simulation locale, on doit attendre le fill pour éviter positions non désirées")
                ...
                for attempt in range(timeout_attempts):
                    ...
                    if parent_ack:
                        ...
                        if order_status == 7:  # FILLED
                            parent_filled = True
                            logger.info(f"✅ Parent LIMIT FILLED - Position ouverte ! Envoi TP/SL maintenant.")
                            break
                        elif order_status in (3, 4):  # PENDING ou OPEN
                            # Afficher progression tous les 5s
                            if attempt % 50 == 0 and attempt > 0:
                                elapsed = attempt * 0.1
                                logger.info(f"⏳ Attente fill... ({elapsed:.0f}s écoulées)")
                            continue
                        else:
                            logger.warning(f"⚠️ Parent status inattendu: {order_status}")
                            break
                ...
                if not parent_filled:
                    logger.error(f"❌ Parent LIMIT non FILLED après 60s - ANNULATION (sécurité)")
                    logger.error(f"❌ Si on envoyait TP/SL, risque de position non désirée !")
                    
                    # Annuler le parent LIMIT qui reste en attente
                    logger.warning(f"🔥 Annulation du parent LIMIT {parent_cid}...")
                    await self._cancel_order_by_client_id(
                        symbol=symbol,
                        client_order_id=parent_cid,
                        reason="Timeout WAIT & FIRE - parent LIMIT non rempli après 60s"
                    )
                    
                    return {
                        "error": "parent_limit_timeout",
                        "message": "Parent LIMIT pas rempli après 60s - Annulé pour sécurité (WAIT & FIRE)",
                        "parent": parent_cid
                    }
        ...
        if mode_str in ("oco206", "childrenmode.oco206"):
            ...
            try:
                ...
                if tp_ack:
                    ...
                    # 🔥 Enregistrer ServerOrderID pour annulation future
                    self._server_order_ids[tp_cid] = tp_ack.get("ServerOrderID", "")
                if sl_ack:
                    logger.info(f"[DTC<-] SL ACK: {sl_ack}")
                    # 🔥 Enregistrer ServerOrderID pour annulation future
                    self._server_order_ids[sl_cid] = sl_ack.get("ServerOrderID", "")

                # 🔥 Enregistrer paire OCO pour gestion automatique
                ...
                if sl_ack:
                    ...
                    if txt.startswith("rejected") or sl_ack.get("Result") == 0:
                        return {"error": "sl_rejected", "details": sl_ack, "parent": parent_cid}
            finally:
