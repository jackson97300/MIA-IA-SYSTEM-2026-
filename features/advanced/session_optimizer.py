...
class SessionOptimizer:
    ...
    def get_current_session_multiplier(self, timestamp: Optional[datetime] = None) -> SessionOptimizationResult:
        ...
        try:
            ...
            # Gestion spéciale pour WEEKEND (pas dans les dictionnaires)
            if current_session == TradingSession.WEEKEND:
                session_metrics = SessionMetrics(
                    avg_volume=0, avg_volatility=0, win_rate=0.0,
                    avg_move_size=0, reversal_frequency=0, breakout_success_rate=0,
                    optimal_holding_time=0, liquidity_score=0.0
                )
    ...
    def _analyze_current_session(self, timestamp: datetime) -> Dict:
        ...
        # Gestion spéciale WEEKEND (pas de définition de session)
        if current_session == TradingSession.WEEKEND:
            return {
    ...
    def _calculate_session_confidence(self, timestamp, session: TradingSession) -> float:
        ...
        # Gestion WEEKEND
        if session == TradingSession.WEEKEND:
            return 1.0  # Confiance maximale pour weekend (facile à détecter)
