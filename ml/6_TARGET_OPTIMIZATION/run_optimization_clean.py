...
Ce script lance l'optimisation compl?te des 8 targets ML
et s?lectionne celle qui maximise le P&L net out-of-sample.
...
Exemples:
  ...
  python run_optimization.py --data /path/to/labeled_trades.parquet
    ...
    parser.add_argument(
        ...
        help='Chemin vers labeled_trades.parquet (d?faut: ml/2_LABELING/labeled_trades.parquet)'
    )
    ...
    parser.add_argument(
        ...
        help='Dossier de sortie pour r?sultats (d?faut: ml/6_TARGET_OPTIMIZATION/results)'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.62,
        help='Fees par trade en ticks (d?faut: 0.62)'
    )

    parser.add_argument(
        '--fast',
        action='store_true',
        help='Mode rapide: subset de donn?es pour test (utiliser avec --n_trades)'
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help='Targets sp?cifiques ? tester (ex: T1_binary_simple T3_pnl_ratio_reg)'
    )
...
def main():
    ...
    # Mode rapide: subset des donn?es
    if args.fast and args.n_trades:
        logger.info(f"??  Mode RAPIDE: Limitation ? {args.n_trades} trades...")
        import pandas as pd
        ...
        logger.info(f"   ? Subset cr??: {len(df_subset)} trades")
    ...
    try:
        ...
        logger.info("="*70)
        logger.info(f"   Dur?e: {elapsed/60:.1f} minutes")
        logger.info(f"   Targets test?es: {len(all_results)}")
        ...
        # G?n?rer visualisations (TODO: ? impl?menter)
        logger.info("   Visualisations: NON IMPLEMENTEES (voir TODOs)")
        ...
        # G?n?rer rapport final (TODO: ? impl?menter)
        logger.info("   Rapport MD: NON IMPLEMENTE (voir ml/DOCS/TARGET_OPTIMIZATION_RESULTS_15NOV.md)")
        # optimizer.generate_markdown_report(all_results, best_target, validation_results)

        logger.info("\n" + "="*70)
        logger.info("?? R?SULTATS DISPONIBLES:")
        logger.info("="*70)
        ...
        logger.info(f"   ?? Tous r?sultats: {output_dir / 'all_results.json'}")
        logger.info(f"   ?? Rapport complet: ml/DOCS/TARGET_OPTIMIZATION_RESULTS_15NOV.md")
        logger.info("")

        # Recommandations
        logger.info("?? PROCHAINES ?TAPES:")
        logger.info("   1. Lire le rapport: TARGET_OPTIMIZATION_REPORT.md")
        logger.info("   2. Valider avec ?quipe trading")
        logger.info("   3. Impl?menter dans bot production")
