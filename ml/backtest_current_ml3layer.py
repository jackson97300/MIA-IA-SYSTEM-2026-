"""
Backtest ML_3LAYER avec Configuration ACTUELLE (Baseline)
==========================================================

Configuration:
- use_fixed_tp_sl = False (ATR adaptatif)
- SL: 1.5x ATR, cappé min/max (20-40t ES, 20-48t NQ)
- TP1: MenthorQ Levels (call_resistance/put_support)
- TP2: GEX Level ou 5x ATR

Objectif:
- Établir baseline performance avec config originale
- Comparer avec backtest optimisé (TP/SL fixes)
...
class ML3LayerBaselineBacktest:
    """
    Backtest ML_3LAYER avec configuration ACTUELLE (ATR adaptatif)
    """
    ...
    def __init__(self, data_path: str = "ml/data/labeled_trades.parquet"):
        ...
        # Configuration ACTUELLE (ATR adaptatif)
        self.sl_min_ticks = {
            'ES': 20,
            'NQ': 20,
            'RTY': 15
        ...
        self.tp_atr_multiplier = 5.0  # Pour TP2
        ...
        logger.info("ML_3LAYER Baseline Backtest initialisé")
        logger.info(f"Config: ATR adaptatif (SL min: ES={self.sl_min_ticks['ES']}t, NQ={self.sl_min_ticks['NQ']}t)")
    ...
    def calculate_sl_baseline(self, row):
        """
        Calcule SL selon logique ACTUELLE (ATR adaptatif)

        Logique:
            - SL = 1.5x ATR
            - Cappé entre sl_min_ticks et sl_max_ticks
        """
        symbol = row['symbol']
        atr = row.get('atr', 20.0)
        ...
        # Appliquer min/max
        sl_min = self.sl_min_ticks.get(symbol, 20)
        sl_max = self.sl_max_ticks.get(symbol, 40)

        sl_ticks = max(sl_min, min(sl_ticks_calculated, sl_max))

        return sl_ticks

    def calculate_tp_baseline(self, row):
        """
        Calcule TP selon logique ACTUELLE (MenthorQ Levels ou ATR)

        Pour simplification backtest:
            - TP1 ≈ VWAP + 2*ATR (LONG) ou VWAP - 2*ATR (SHORT)
            - TP2 ≈ 5x ATR
            - Utiliser moyenne TP = 3.5x ATR comme proxy
        """
        atr = row.get('atr', 20.0)
        tick_size = 0.25

        # TP moyen ≈ 3.5x ATR (entre TP1=2x et TP2=5x)
        tp_distance_pts = atr * 3.5
        tp_ticks = int(tp_distance_pts / tick_size)

        # Limiter TP selon symbole
        symbol = row['symbol']
        max_tp = {'ES': 60, 'NQ': 80, 'RTY': 70}.get(symbol, 60)
        tp_ticks = min(tp_ticks, max_tp)

        return tp_ticks

    def simulate_trade(self, row):
        """
        Simule un trade avec config BASELINE
        ...
        # Calculer SL et TP baseline
        sl_ticks = self.calculate_sl_baseline(row)
        tp_ticks = self.calculate_tp_baseline(row)
    ...
    def run_backtest(self):
        ...
        """
        logger.info("Lancement du backtest BASELINE...")
    ...
    def generate_report(self, metrics, output_path: str = "ml/output/backtest_baseline_report.txt"):
        """
        ...
        report = []
        report.append("=" * 70)
        report.append("BACKTEST ML_3LAYER - CONFIGURATION BASELINE (ATR ADAPTATIF)")
        report.append("=" * 70)
        report.append("")
        report.append(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report.append("")
        report.append("CONFIGURATION:")
        report.append("  SL: 1.5x ATR, cappé min/max (ES: 20-40t, NQ: 20-48t)")
        report.append("  TP: MenthorQ Levels ou 3.5x ATR moyen")
        ...
        for symbol in ['ES', 'NQ', 'TOTAL']:
            ...
            report.append(f"  P&L/trade:         {m['pnl_per_trade_ticks']:+.3f} ticks")
            report.append("")
...
def main():
    ...
    Fonction principale
    """
    logger.info("DÉMARRAGE BACKTEST BASELINE ML_3LAYER")
    logger.info("=" * 70)

    # Initialiser backtester
    backtester = ML3LayerBaselineBacktest(
    ...
    output_csv = Path("ml/output/backtest_baseline_results.csv")
    ...
    logger.info("")
    logger.info("=" * 70)
    logger.info("BACKTEST BASELINE TERMINÉ")
    logger.info("=" * 70)
