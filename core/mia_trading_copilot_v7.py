...
# ═══════════════════════════════════════════════════════════════
# 🔄 ROLLOVER AUTOMATIQUE - Plus jamais de problème de contrat!
# ═══════════════════════════════════════════════════════════════
def get_current_contract_month(date: Optional[datetime] = None) -> str:
    """
    Détermine automatiquement le code du contrat actif (H, M, U, Z).

    Calendrier des contrats CME:
    - H = Mars (expire 3ème vendredi mars)
    - M = Juin (expire 3ème vendredi juin)
    - U = Septembre (expire 3ème vendredi septembre)
    - Z = Décembre (expire 3ème vendredi décembre)

    Rollover typique: ~8-10 jours avant expiration (vers le 10 du mois)

    Returns: Code contrat + année (ex: "H25", "M25", "Z25")
    """
    if date is None:
        date = datetime.now()

    month = date.month
    year = date.year % 100  # 2025 -> 25

    # Mapping mois -> contrat actif (après rollover du 10 du mois)
    # Rollover le ~10 du mois d'expiration vers le contrat suivant
    if month == 12:
        # Décembre: avant le 10 = Z actuel, après le 10 = H année suivante
        if date.day < 10:
            return f"Z{year}"
        else:
            return f"H{(year + 1) % 100}"
    elif month in [1, 2]:
        # Jan-Fév: H (Mars)
        return f"H{year}"
    elif month == 3:
        # Mars: avant le 10 = H, après le 10 = M
        if date.day < 10:
            return f"H{year}"
        else:
            return f"M{year}"
        ...
        # Juin: avant le 10 = M, après le 10 = U
        if date.day < 10:
            return f"M{year}"
        else:
            return f"U{year}"
    elif month in [7, 8]:
        # Juil-Août: U (Septembre)
        return f"U{year}"
    elif month == 9:
        # Sept: avant le 10 = U, après le 10 = Z
        if date.day < 10:
            return f"U{year}"
        else:
            return f"Z{year}"
...
def build_snapshot_path(symbol: str, date: Optional[datetime] = None) -> Optional[Path]:
    ...
    # 🔄 ROLLOVER AUTOMATIQUE - Plus de problème de contrat!
...
# ✅ TODO #7: Parsing logs robuste - Format MIA réel
def parse_trade_log_line(line: str) -> Optional[Dict]:
    """Parse une ligne de log trade - Format MIA: '08:13:14 - INFO - [NQ] ENTRY | {...}'"""
    try:
        if not line or not line.strip():
            return None

        # Format MIA réel: "HH:MM:SS - INFO - [SYMBOL] ACTION | {json...}"
        # Exemple: "08:13:14 - INFO - [NQ] ENTRY | {'direction': 'LONG', ...}"

        # Pattern pour extraire les composants
        match = re.match(r'^(\d{2}:\d{2}:\d{2})\s*-\s*\w+\s*-\s*\[(\w+)\]\s*(\w+)\s*\|\s*(.+)$', line.strip())

        if not match:
            # Fallback: ancien format "HH:MM:SS | SYMBOL | ACTION | {json}"
            parts = line.split(' | ')
            if len(parts) >= 3:
                ...
                if re.match(r'\d{2}:\d{2}:\d{2}', time_str):
                    ...
                    if symbol in ['ES', 'NQ', 'RTY'] and action in ['ENTRY', 'EXIT']:
                        data = {}
                        if len(parts) >= 4:
                            try:
                                # Remplacer single quotes par double quotes pour JSON valide
                                json_str = parts[3].replace("'", '"')
        ...
        # Valider action
        if action not in ['ENTRY', 'EXIT', 'OPEN', 'CLOSE']:
            return None

        # Parser le JSON (attention: utilise single quotes au lieu de double quotes)
        data = {}
        try:
            # Remplacer single quotes par double quotes pour JSON valide
...
def load_trades_from_log(date: datetime) -> List[Dict]:
    """Charge les trades depuis le fichier log - Format MIA"""
    date_str = date.strftime("%Y%m%d")

    # Chercher dans logs_advanced/trades/ d'abord
    log_path = PROJECT_ROOT / "logs_advanced" / "trades" / f"trades_{date_str}.log"

    if not log_path.exists():
        # Fallback: chercher dans logs/
        log_path = PROJECT_ROOT / "logs" / f"trades_{date_str}.log"

    if not log_path.exists():
        return []

    trades = []
    entries = {}  # Pour matcher ENTRY avec EXIT
    ...
    try:
        with open(log_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if action == 'ENTRY':
                    # Stocker l'entrée en attente de la sortie
                    entries[symbol] = {
                        'symbol': symbol,
                        ...
                        'strategy': data.get('strategy', 'ML_3Layer'),
                    }
                    ...
                    if symbol in entries:
                        ...
                        # Exit sans entry correspondante - créer trade partiel
                        trade = {
                            'symbol': symbol,
                            'time_entry': 'N/A',
                            'direction': data.get('direction', 'LONG'),
        ...
        st.warning(f"Erreur lecture logs: {e}")
...
def render_header(session: Dict):
    ...
    import base64
    logo_path = PROJECT_ROOT / "assets" / "logo_mia_dark.jpg.jpeg"
    logo_base64 = ""
    try:
        if logo_path.exists():
        ...
        pass  # Fallback sans logo si erreur

    logo_html = ""
    if logo_base64:
        logo_html = f'<img src="data:image/jpeg;base64,{logo_base64}" style="width: 90px; height: 90px; object-fit: contain; mix-blend-mode: screen; filter: brightness(1.1) contrast(1.2);">'

    st.markdown(f"""
    <div class="main-header">
        <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 12px;">
            <div style="width: 90px; height: 90px; display: flex; align-items: center; justify-content: center;">
                {logo_html if logo_html else '<div style="width: 90px; height: 90px;"></div>'}
            </div>
            <div>
                <div class="main-title" style="font-size: 2rem; background: linear-gradient(90deg, #4a9eff, #0fbf84);
                                                 -webkit-background-clip: text; -webkit-text-fill-color: transparent;
                                                 font-weight: 800; letter-spacing: 2px;">
                    MIA IA SYSTEM
                </div>
                <div style="font-size: 0.9rem; color: #64748b; font-weight: 500; margin-top: 2px;">
                    Trading Copilot V7 - Intelligence Artificielle
                </div>
            </div>
        </div>
...
def render_tab_analyse():
    ...
    with col1:
        ...
        <div class="trade-row {'win' if trade['result'] == 'WIN' else 'loss'}">
            ...
            <div style="margin-top: 8px; color: #94a3b8; font-size: 0.9rem;">
                ...
                Duration: {int(trade.get('duration_ms', 0)/1000//60)}m {int(trade.get('duration_ms', 0)/1000%60)}s
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # 🔧 12/12: Convertir MFE/MAE de USD en ticks
        symbol = trade.get('symbol', 'ES')
        tick_value = {'ES': 12.50, 'NQ': 5.00, 'RTY': 5.00}.get(symbol, 12.50)
        mfe_usd = trade.get('mfe', 0)
        mae_usd = trade.get('mae', 0)
        mfe_ticks = mfe_usd / tick_value if tick_value else 0
        mae_ticks = mae_usd / tick_value if tick_value else 0

        with mcol1:
            render_metric("P&L Ticks", f"{trade.get('pnl_ticks', 0):+.1f}t",
                         'green' if trade.get('pnl_ticks', 0) > 0 else 'red')
        with mcol2:
            render_metric("MFE", f"{mfe_ticks:+.1f}t", 'blue')
        with mcol3:
            render_metric("MAE", f"{mae_ticks:.1f}t", 'red' if mae_ticks < 0 else 'yellow')
    ...
    with col2:
        ...
        # MFE vs PnL (efficacité) - 🔧 12/12: Corrigé pour utiliser les ticks
        mfe_usd = trade.get('mfe', 0)
        pnl_ticks = trade.get('pnl_ticks', 0)
        # Convertir MFE de USD en ticks pour comparaison
        symbol = trade.get('symbol', 'ES')
        tick_value = {'ES': 12.50, 'NQ': 5.00, 'RTY': 5.00}.get(symbol, 12.50)
        mfe_ticks = mfe_usd / tick_value if tick_value else 0
        ...
        if mfe_ticks > 0:
            if pnl_ticks > 0:
                # Trade gagnant: calculer efficacité de capture
                efficiency = pnl_ticks / mfe_ticks * 100
                if efficiency >= 50:
                    analysis.append(('✅', f"Bonne capture MFE ({efficiency:.0f}%)", 'ok'))
                    quality_score += 1
                else:
                    analysis.append(('⚠️', f"MFE partiellement capturé ({efficiency:.0f}%)", 'warn'))
            else:
                # 🔧 12/12: Trade perdant avec MFE positif = profit non sécurisé
                analysis.append(('❌', f"MFE +{mfe_ticks:.0f}t non sécurisé → SL touché", 'fail'))
                quality_score -= 1
        elif pnl_ticks < 0:
            # Trade perdant sans MFE = jamais en profit
            analysis.append(('❌', f"Jamais en profit (MFE: 0t)", 'fail'))
...
def render_tab_config():
    ...
    with col2:
        ...
        for name, info in TRADING_SESSIONS.items():
            if name == 'HARD_STOP':
                continue
            icon = '✅' if info.get('tradable', False) else '❌'
            start_h = info.get('start_hour', 0)
            start_m = info.get('start_min', 0)
            end_h = info.get('end_hour', 0)
            end_m = info.get('end_min', 0)
            time_str = f"{start_h:02d}:{start_m:02d} - {end_h:02d}:{end_m:02d}"
            st.markdown(f"""
            <div style="background: #0f1620; padding: 12px; border-radius: 8px; margin: 8px 0;
                        border-left: 4px solid {info.get('color', '#64748b')};">
                {icon} <strong>{name}</strong> ({time_str})<br>
                <span style="color: #64748b;">Quality:</span> <strong>{info.get('quality', 0)}</strong>
            </div>
            """, unsafe_allow_html=True)
