...
std::string BuildMLReadyJSON15(
  ...
  os << "\"dom_bid_1\":" << format_size(dom_bid_qty[0]) << ","
     ...
     << "\"position_in_range\":" << format_ratio(position_in_range) << ","
       ...
       << "\"feature_version\":\"v3.5.21_day_change_removed\""
      << "}\n";
   return os.str();
    // PATCH <<< build JSON
}
...
inline double NormalizePx(const SCStudyInterfaceRef& sc, double raw)
{
  ...
  // Si mult < 1.0 ET raw est dans une gamme "humaine" (< 10000), Sierra a déjà appliqué le multiplier
  // Sinon (raw > 10000), c'est le format brut, on doit diviser
  if (mult < 1.0 && raw < 10000.0) {
    // Prix déjà normalisé (ex: GC=3948.6, CL=68.5) → NE PAS diviser
    px = raw;
  } else {
    // Prix brut (ex: ES=675650, NQ=2531775, RTY=242440) → Diviser par mult
    px = raw / mult;
  ...
  // 3) Cas spécial pour multiplier <= 0.1 (GC=0.1, RTY=0.01, CL=0.01) :
  // Logique intelligente basée sur la GAMME DE PRIX typique :
  // - ES/NQ (mult=1.0): Prix > 100000 après ÷mult → diviser par 100
  // - RTY (mult=0.01): Prix typique ~2420, après ÷mult = 242000 → ÷100 = 2420 ✅ PAS de division supplémentaire
  // - GC (mult=0.1): Prix typique ~2640, après ÷mult = 26400 → ÷100 = 264 ❌ puis besoin ÷10 → 26.4 ❌
  //                  → Si mult=0.1 ET px > 10000, diviser par 10
  // - CL (mult=0.01): Prix typique ~68, après ÷mult = 6800 → ÷100 = 68 ✅ PAS de division supplémentaire
  //
  // CONCLUSION : Après l'auto-detect ×100, si mult <= 0.1 ET px > 10000 → diviser selon mult
  //              Sinon (px < 10000), le prix est déjà correct !
  if (mult <= 0.1 && px > 10000.0) {
