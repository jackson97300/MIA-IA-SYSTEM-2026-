...
std::string BuildMLReadyJSON15(
  ...
  auto format_ms = [](int64_t tms) -> std::string {
    ...
    os << "{"
       ...
       << "\"deltaPct\":" << calc_deltaPct_formatted << ","
...
struct UnifiedState {
  ...
  double vwap_weekly=0, vwap_weekly_up1=0, vwap_weekly_dn1=0;  // ← NOUVEAUX
  double vwap_monthly=0;  // ← NOUVEAUX
                          ...
                          unsigned seq_unified)
{
  ...
  live_json.Format(
    ...
    R"("vwap":%.2f,"vwap_weekly":%.2f,"vwap_weekly_up1":%.2f,"vwap_weekly_dn1":%.2f,"vwap_monthly":%.2f,)"
    R"("vah":%.2f,"val":%.2f,"vpoc":%.2f,"d_vwap_ticks":%.2f,"d_vpoc_ticks":%.2f,)"
    ...
    live.vwap, live.vwap_weekly, live.vwap_weekly_up1, live.vwap_weekly_dn1, live.vwap_monthly,
    live.vah, live.val, live.vpoc, live.d_vwap_ticks, live.d_vpoc_ticks,
                             ...
                             unsigned seq_unified)
{
  ...
  // ================== GARDE-FOUS POUR DONNÉES DE QUALITÉ ===================
  // Détection des conditions de marché fermé/stale
  bool closed = is_closed_or_stale(U, 0, 0, 1.0, false, 0, U.bid, U.ask, "QUOTE", U.dom_bq1, U.dom_aq1);

  // Tentative de réparation BBO
  double bid = U.bid, ask = U.ask;
  bool bbo_repaired = repair_bbo(bid, ask, U, t_ms);

  // Si BBO non réparable, émettre heartbeat minimal
  if (!bbo_repaired) {
    // Heartbeat minimal pour télémétrie
    FILE* f = fopen(ML_FilePath(chart, sym).GetChars(), "a");
    if (f) {
      fprintf(f, "{\"t_ms\":%lld,\"sym\":\"%s\",\"chart\":%d,\"emit_reason\":\"heartbeat\",\"data_quality\":\"CLOSED_OR_STALE\"}\n",
              t_ms, sym, chart);
      fclose(f);
    }
    return; // Ne pas calculer les features dépendantes
  }

  // Recalculer mid/spread après réparation
  const double mid = 0.5*(bid + ask);
  const double spread = std::max(0.0, U.ask - U.bid);
  const int    spread_ticks = (int)llround(ml_safe_div(spread, tick));
  const double microprice = (U.bid*U.aq + U.ask*U.bq) > 0 ? ( (U.bid*(double)U.aq + U.ask*(double)U.bq)/ (double)(U.aq+U.bq) ) : mid;
  ...
  // === CALCUL DES MÉTRIQUES DE SESSION (NOUVEAU SYSTÈME) ===
  int elapsed_s=0; double progress01=0.0;
  ...
  std::string ml15_line = BuildMLReadyJSON15(
    ...
    U.dom_bid1, U.dom_ask1, U.dom_bq1, U.dom_aq1, // DOM L1
...
struct StudyIds {
  ...
  int vwap_weekly = 0, vwap_monthly = 0;  // ← NOUVEAUX
  bool resolved = false;
};
...
static void ResolveStudiesForChart(const SCStudyInterfaceRef& sc) {
  ...
  // PATCH: Fallback VWAP Daily pour tous les charts si résolution par nom échoue
  if (ids.vwap <= 0) {
    if (sc.ChartNumber == 3) {
      ids.vwap = 22;  // ES: ID 22
    } else if (sc.ChartNumber == 9) {
      ids.vwap = 22;  // NQ: ID 22
    }
    // Log du fallback
    SCString path;
    ...
    if (f) {
      fprintf(f, "[%.6f] VWAP Daily fallback: Chart %d -> ID 22\n",
  ...
  // Fallback vers les IDs fixes selon le chart
  ...
  FILE* f = fopen(path.GetChars(), "a");
  if (f) {
    fprintf(f, "[%.6f] Chart %d -> vwap=%d pvwap=%d vva=%d nbcv=%d atr=%d corr=%d vix=%d gamma=%d blind=%d\n",
      sc.CurrentSystemDateTime.GetAsDouble(),
      sc.ChartNumber, ids.vwap, ids.pvwap, ids.vva, ids.nbcv, ids.atr, ids.corr, ids.vix, ids.gamma, ids.blind_spots);
    fclose(f);
  }
}
...
inline double NormalizePx(const SCStudyInterfaceRef& sc, double raw)
{
  // 1) DÃ©-multiplier si besoin
  const double mult = (sc.RealTimePriceMultiplier != 0.0 ? sc.RealTimePriceMultiplier : 1.0);
  double px = raw / mult;

  // 2) Auto-dÃ©tection du format x100 (ES/NQ typiquement 6000-8000 en humain, 600000-800000 en x100)
  // Si l'input 47 est activÃ© manuellement, l'utiliser
  if (sc.Input[47].GetYesNo()) {
    px /= 100.0;
  } else {
    // Auto-dÃ©tection: si le prix semble Ãªtre en format x100 (trÃ¨s grand), diviser par 100
...
SCSFExport scsf_MIA_Dumper_G3_Unifier(SCStudyInterfaceRef sc)
{
  if (sc.SetDefaults)
  {
    ...
    sc.Input[34].SetInt(7); // Study ID 7 pour MenthorQ Gamma Levels (CHART_3)
  ...
  if (sc.Input[2].GetInt() != 0 && sc.ArraySize > 0) {
    ...
    // === VWAP WEEKLY/MONTHLY ===
    const StudyIds& ids = g_study_by_chart[sc.ChartNumber];
    double vwap_weekly = 0, vwap_weekly_up1 = 0, vwap_weekly_dn1 = 0;
    double vwap_monthly = 0;

    // VWAP Weekly
    if (ids.vwap_weekly > 0) {
      SCFloatArray VWAP_WEEKLY, VWAP_WEEKLY_UP1, VWAP_WEEKLY_DN1;
      ReadSubgraph(sc, ids.vwap_weekly, VWAP_SG_MAIN, VWAP_WEEKLY);
      ReadSubgraph(sc, ids.vwap_weekly, VWAP_SG_UP1, VWAP_WEEKLY_UP1);
      ReadSubgraph(sc, ids.vwap_weekly, VWAP_SG_DN1, VWAP_WEEKLY_DN1);

      if (ValidateStudyData(VWAP_WEEKLY, i)) {
        vwap_weekly = VWAP_WEEKLY[i];
      }
      if (ValidateStudyData(VWAP_WEEKLY_UP1, i)) {
        vwap_weekly_up1 = VWAP_WEEKLY_UP1[i];
      }
      if (ValidateStudyData(VWAP_WEEKLY_DN1, i)) {
        vwap_weekly_dn1 = VWAP_WEEKLY_DN1[i];
      }

      // PATCH: Correction des bandes Weekly inversées
      if (std::isfinite(vwap_weekly) && std::isfinite(vwap_weekly_up1) && std::isfinite(vwap_weekly_dn1)) {
        if (vwap_weekly_up1 < vwap_weekly && vwap_weekly_dn1 > vwap_weekly) {
          // Les bandes sont inversées, on les swap
          std::swap(vwap_weekly_up1, vwap_weekly_dn1);
          // Log de correction (une seule fois par session)
          static bool weekly_swap_logged = false;
          if (!weekly_swap_logged) {
            // Log via fichier de debug
            SCString path;
            path.Format("D:\\MIA_IA_system\\study_resolve_chart_%d.log", sc.ChartNumber);
            FILE* f = fopen(path.GetChars(), "a");
            if (f) {
              fprintf(f, "[%.6f] WARNING: VWAP Weekly bands inverted - corrected automatically\n",
                sc.CurrentSystemDateTime.GetAsDouble());
              fclose(f);
            }
            weekly_swap_logged = true;
          }
        }
      }
    }
    ...
    // VWAP Monthly
    if (ids.vwap_monthly > 0) {
      SCFloatArray VWAP_MONTHLY;
      ReadSubgraph(sc, ids.vwap_monthly, VWAP_SG_MAIN, VWAP_MONTHLY);
      if (ValidateStudyData(VWAP_MONTHLY, i)) {
        vwap_monthly = VWAP_MONTHLY[i];
      }
    } else {
      // PATCH: Si Monthly non trouvé, on laisse à NaN (sera null en JSON)
      vwap_monthly = std::numeric_limits<double>::quiet_NaN();
    }

    // Mettre à jour UnifiedState
    UnifiedState& U = g_UState[sc.Symbol.GetChars()];
    U.vwap_weekly = vwap_weekly;
    U.vwap_weekly_up1 = vwap_weekly_up1;
    U.vwap_weekly_dn1 = vwap_weekly_dn1;
    U.vwap_monthly = vwap_monthly;
      }
    }
  }

  // ========== VVA (Volume Value Area Lines) - avec dÃ©duplication amÃ©liorÃ©e ==========
    ...
    if (last != last_pvwap_bar_by_chart[sc.ChartNumber] || !pvwap_executed_today_by_chart[sc.ChartNumber])
    {
      ...
      if (currStart > 0) {
        ...
        if (sumV > 0.0) {
          ...
          // ===== STOCKER LE PVWAP RÉEL DANS UNIFIEDSTATE =====
          UnifiedState& U = g_UState[sc.Symbol.GetChars()];
          U.pvwap = pvwap;
          U.p_up1 = up1; U.p_dn1 = dn1; U.p_up2 = up2; U.p_dn2 = dn2;

          // Harmonisation d'Ã©chelle: si l'autoscale x100 est actif ailleurs,
  ...
  if (sc.Input[67].GetInt() != 0) {
    ...
    if (should_write_unified) {
      // Désactiver l'écriture legacy pour CHART 3 et CHART 9 pour éviter doublons avec WriteUnified
      if (sc.ChartNumber == 9 || sc.ChartNumber == 3) {
        // Le CHART 9 utilise exclusivement WriteUnified(sc, sym)
  ...
  if (/*force MenthorQ Gamma*/ true && sc.ArraySize > 0) {
    ...
    if (ids.resolved && ids.gamma > 0) {
      ...
      // Fallback vers l'ancien système
      menthorqGammaID = sc.Input[34].GetInt(); // Study ID 7 (CHART_3) ou 47 (CHART_9)
      if (menthorqGammaID <= 0) {
        if (sc.ChartNumber == 3) menthorqGammaID = 7;
        else if (sc.ChartNumber == 9) menthorqGammaID = 47;
        else menthorqGammaID = 7;
      }
    }
  ...
  // === PATCH 5: Unified cadence trigger START ===
  {
    ...
    if (sc.ChartNumber != 3 && sc.ChartNumber != 9) {
      WriteUnified(sc, sym);
    }
  }
